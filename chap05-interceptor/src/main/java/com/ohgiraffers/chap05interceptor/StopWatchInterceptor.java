...
* JHandlerInterceptorAdaptor를 이용해 사용했으나, default 매소드가 인터페이스에
* 사용 가능하게된 java 8 버전 이후에는 인터페이스만 구현하여 필요한 메소드만 오버라이딩 해서 사용할 수 있다.
* */

@Component
public class StopWatchInterceptor implements HandlerInterceptor {

    // 인터페이스는 다 구헌해야 사용할 수 있었음. default

    // 전처리 매소드 - 지정된 컨트롤러의 동작 이전에 수행할 내용
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        // post 가 보낸 파라미터가 있음
        if (!request.getParameter("auth").equals("admin")) {
            response.sendRedirect("/"); // 루트 경로 redirect
